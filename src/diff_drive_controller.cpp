  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  { 
    ...
    if (params_.odom_middle_wheel_calc)
    {
      ...
      {
        idx = static_cast<size_t>(params_.wheels_per_side / 2);
        if (idx >= static_cast<size_t>(params_.wheels_per_side))
        {
          idx = static_cast<size_t>(params_.wheels_per_side - 1);
        }
      }
      
      const double left_feedback =
        registered_left_wheel_handles_[idx].feedback.get().get_value();
      const double right_feedback =
        registered_right_wheel_handles_[idx].feedback.get().get_value();

      if (std::isnan(left_feedback) || std::isnan(right_feedback))
      {
        RCLCPP_ERROR(
          logger,
          "Odom middle-wheel: either the left or right wheel %s is invalid for index [%zu]",
          feedback_type(), idx);
        return controller_interface::return_type::ERROR;
      }
    ...
    else
    {
      for (size_t index = 0; index < static_cast<size_t>(params_.wheels_per_side); ++index)
      {
        ...
        {
          RCLCPP_ERROR(
            logger, "Either the left or right wheel %s is invalid for index [%zu]",
            feedback_type(), index);
          return controller_interface::return_type::ERROR;
        }
